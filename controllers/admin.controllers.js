...
exports.LoginAdmin = async (request,response) => {
    try {
        ...
        if(!findAdmin){
            response.status(400).send({ errors : [{msg : 'email or password invalid'}]})
        }

        const testPassword = bcrypt.compareSync (password , findAdmin.password)

        if(!testPassword){
            response.status(400).send({ errors : [{msg : 'email email or password invalid'}]})
        }
        const token = jwt.sign({
            _id: findAdmin._id
          }, process.env.SECRET_KEY, { expiresIn: '5h' });

        return response.status(200).send({msg : 'login secc' , Admin : findAdmin , token})
    }
    catch (error) {
