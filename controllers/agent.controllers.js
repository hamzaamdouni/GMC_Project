...
exports.RegisterAgent = async (request,response) => {
    try {
        ...
        if(findUser){
            ...
            const newAgent = new agent({...request.body , id_agent : _id , id_service : findCategory.id_service , id_category : findCategory._id })
            await newAgent.save()
...
exports.UpdateAgent = async (request , response) => {
    try {
        await agent.updateOne({ id_agent : request.user._id }, { $set: { ...request.body } });
...
exports.TraiterDemande = async (request,response) => {
    try {
        ...
        await demande.updateOne({_id : iddemande } , { $set : {...request.body.etat}})
        response.send({ msg: "updated succ" });
    } catch (error) {
      console.log(error);
...
exports.TraiterComment = async (request,response) => {
    try {
        ...
        await comment.updateOne({_id : idcomment } , { $set : {...request.body.etat}})
        response.send({ msg: "updated succ" });
    } catch (error) {
      console.log(error);
